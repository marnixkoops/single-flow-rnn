...
padded_sequences_train = padded_sequences.copy()
...
if LOGGING:
    mlflow.start_run(experiment_id=2)  # start mlflow run for experiment tracking
...
def embedding_GRU_model(
    hp, batch_size=BATCH_SIZE, vocab_size=N_TOP_PRODUCTS,
):
    model = tf.keras.Sequential(
        [
            tf.keras.layers.Embedding(  # we can also try hashing instead of embedding
                input_dim=N_TOP_PRODUCTS,
                output_dim=hp.Int("EMBED_DIM", min_value=32, max_value=256, step=16),
                batch_input_shape=[BATCH_SIZE, None],
                mask_zero=True,
            ),
            tf.keras.layers.GRU(
                units=hp.Int("N_HIDDEN_UNITS", min_value=32, max_value=1024, step=32),
                dropout=hp.Choice("DROPOUT", [0.0, 0.1, 0.25, 0.4]),
                recurrent_dropout=hp.Choice("RECURRENT_DROPOUT", [0.0, 0.1, 0.25, 0.4]),
...
tuner = Hyperband(
    ...
    project_name="GRU-Embedding-Hyperband",
)

tuner.search_space_summary()

tuner.search(x=X_train, y=y_train, epochs=3, validation_data=(X_val, y_val))
...
print(
    "⏱️ Elapsed time for tuning hyperparameters on {} sequences: {:.3} minutes".format(
