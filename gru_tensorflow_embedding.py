...
DOUBLE_DATA = False  # loads two weeks worth of raw data instead of 1
...
print("[⚡] Tokenizing, padding & filtering sequences")
...
# delete all sequences with only 1 product, these contain no information. we need at least 2
# products to have an input-output pair to train on. a sequence should have > 1 non-zero element.
invalid_sequence_mask = np.sum((padded_sequences != 0), axis=1) > 1  # create mask
padded_sequences = padded_sequences[invalid_sequence_mask].copy()
print("     Generated {} valid sequences".format(len(padded_sequences)))
...
# clean up memory
del sequence_df, sequences, invalid_sequence_mask


# reshape sequences for model training/validation
def generate_train_test_pairs(array, input_length=4, step_size=1):
...
X_train, y_train = padded_sequences_train[:-val_index], padded_sequences_train[:-val_index, -1]
X_val, y_val = padded_sequences_train[:val_index, :-1], padded_sequences_train[:val_index, -1]
X_test, y_test = padded_sequences_test[:, :-1], padded_sequences_test[:, -1]
...
print("[⚡] Elapsed time for tokenizing, padding and filtering: {}".format(time.time() - t_prep))
...
def embedding_rnn_model(
    input_sequence, target, vocab_size=N_TOP_PRODUCTS, embed_dim=EMBED_DIM, dropout=DROPOUT
):
    ...
    output, state = tf.nn.dynamic_rnn(
        cell, embeddings, dtype=tf.float32  # time_major=False
...
model = tf.contrib.learn.Estimator(
    model_fn=embedding_rnn_model, config=tf.estimator.RunConfig(log_step_count_steps=1000)
)
...
print(
    "[⚡] Elapsed time for training on {} sequences: {:.3} minutes".format(
        len(y_train), (time.time() - t_train) / 60
    )
)

print("\n[⚡] Creating recommendations on test set (logits for all products)")
# to create 1 recommendation per user we only predict on the last 4 items per user sequence
y_pred_probs = np.array([p["prob"] for p in model.predict(X_test[:, -4:], as_iterable=True)])
print(
    "     Elapsed time for predicting {} sequences: {:.3} seconds".format(
        len(y_test), time.time() - t_train
    )
)
...
def generate_predicted_sequences(y_pred_probs, output_length=5):
...
y_pred = np.vstack(predicted_sequences[:, 1])  # first column is the predicted next viewed item
map_k5 = average_precision.mapk(np.vstack(y_test), predicted_sequences, k=5)
score_k1 = accuracy_score(y_test, y_pred)
recom_coverage_k1 = len(np.unique(y_pred)) / N_TOP_PRODUCTS
recom_novelty_k1 = sum([(y_pred[i] not in X_test[i]) for i in range(len(y_test))]) / len(y_test)

print("     MAP @ 5: {:.4}%".format(map_k5 * 100))
print("     Accuracy @ 1: {:.4}%".format(score_k1 * 100))
print("     Coverage @ 1: {:.4}% (unique recom / all input)".format(recom_coverage_k1 * 100))
print("     Novelty @ 1: {:.4}% (recom products not in input)".format(recom_novelty_k1 * 100))
...
if not DRY_RUN:
    ...
    # Set tags
    mlflow.set_tag("run mode", ("DRY_RUN: {}, DOUBLE_DATA: {}".format(DRY_RUN, DOUBLE_DATA)))

    # Log parameters
    mlflow.log_param("n_top_products", N_TOP_PRODUCTS)
    ...
    # Log metrics
    mlflow.log_metric("MAP5", map_k5)
    mlflow.log_metric("accuracy", score_k1)
    mlflow.log_metric("coverage", recom_coverage_k1)
    mlflow.log_metric("novelty", recom_novelty_k1)
    ...
    with open("info.txt", "w") as f:
        ...
        f.write("\n")
        f.write("\n[+] Model definition:")
        f.write("\n{}".format(inspect.getsource(embedding_rnn_model)))

    mlflow.log_artifact("info.txt")

    print("[⚡] Elapsed time for entire flow: {:.3} minutes".format((time.time() - t_prep) / 60))
